...
@torch.no_grad()
def calc_performance(result: dict, result_anchor: dict, dataset_size: int, typ='bd-rate', reduce=True):
    """
    result1 & result2: {
        ...
        for lmbda in lmbdas:
            ...
            for metric_name in result2_sample.keys():
                ...
                if "time" in metric_name.lower():
                    ...
                    delta = (t1-t2) / t2
                    results[metric_name].append(delta)
                else:
                    # Distortion
                    bd = metricfunc(metrics1['bpp'], 
                                    metrics1[metric_name],
                                    metrics2['bpp'],
                                    metrics2[metric_name])
                    results[metric_name].append(bd)
